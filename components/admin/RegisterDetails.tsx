...
import { Search, Download, Trash2, ChevronDown, CheckCircle, XCircle, Mail, X } from "lucide-react";
...
export default function CandidateDashboard() {
  ...
  const filteredData = users.filter((u) => {
    ...
    const matchesTab = activeTab === "email" ? status === "accepted" : status === activeTab;
    const matchesSearch =
  ...
  const getCount = (tabId: string) => {
    return users.filter((u) => (u.status || "pending") === (tabId === "email" ? "accepted" : tabId)).length;
  };
    ...
    <div className="min-h-screen bg-gray-50 p-6 font-sans text-gray-800">
          ...
          >
            ...
            <span
              className={`ml-3 text-xs px-2 py-0.5 rounded-full ${
                activeTab === tab.id ? "bg-white text-[#1e1b4b]" : "bg-gray-100 text-gray-600"
              }`}
            >
              {getCount(tab.id)}
            </span>
      ...
      {activeTab === "email" ? (
        ...
        <TableLayout
          ...
          onViewAadhaar={setSelectedAadhaar}
        />
      )}
  ...
  return (
    <>
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm mb-6 overflow-hidden flex flex-col max-h-[calc(100vh-250px)]">
        <div className="p-4 flex flex-col md:flex-row gap-4 items-center justify-between bg-white z-10">
        ...
        <div className="flex-1 overflow-y-auto min-h-[400px]">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-gray-400">
              <p className="text-lg font-medium text-gray-500 mb-1">No candidates found</p>
            </div>
          ) : (
              ...
              {data.map((item) => (
                ...
                >
                  <div className="col-span-3">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Father: {item.fatherName}</p>
                  </div>
  
                  <div className="col-span-3 overflow-hidden">
                    <p className="text-sm text-gray-700 truncate" title={item.email}>{item.email}</p>
                  ...
                  <div className="col-span-1 flex justify-center">
                    <button
                      ...
                      className="text-[#3B3BB7] hover:text-indigo-900 flex items-center gap-1 group transition-colors"
                      title="View Aadhaar"
                    >
                    ...
                    <div className="flex gap-2">
                      ...
                      {item.status !== "rejected" && (
                        <button
                          ...
                          className="p-1.5 bg-red-50 text-red-600 border border-red-100 rounded-md hover:bg-red-100 transition-colors"
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
        <h2 className="text-[#1e1b4b] font-bold text-lg mb-4">Send Email to Approved Participants</h2>
        <div className="mb-4">
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
        <div className="px-6 py-3 bg-gray-50/50 flex items-center gap-3 border-b border-gray-100">
          <input
            ...
            onChange={(e) =>
              setSelectedEmails(e.target.checked ? data.map((u) => u.email) : [])
            }
            className="w-4 h-4 rounded border-gray-300 text-indigo-900 focus:ring-indigo-500 cursor-pointer"
          />
          <span className="text-sm font-medium text-gray-700">Select All ({data.length})</span>
        </div>
