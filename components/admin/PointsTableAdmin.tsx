...
export default function PointsTableee() {
  ...
  const handleUpdateTable = async () => {
    ...
    const sA = [team1Sets.s1, team1Sets.s2, team1Sets.s3].map((v) =>
      v === "" ? null : Number(v),
    );
    const sB = [team2Sets.s1, team2Sets.s2, team2Sets.s3].map((v) =>
      v === "" ? null : Number(v),
    );
    ...
    const winner =
      setsWonA > setsWonB ? team1 : setsWonB > setsWonA ? team2 : "";
    ...
    const updated = teams.map((t) => {
      if (t.name === team1 || t.name === team2) {
        ...
        const setRatioNew =
          setsLostNew > 0
            ? Number((setsWonNew / setsLostNew).toFixed(2))
            : setsWonNew;
        ...
        const pointRatioNew =
          pointsAgainstNew > 0
            ? Number((pointsForNew / pointsAgainstNew).toFixed(2))
            : pointsForNew;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          data?.error || `Save failed with status: ${res.status}`,
        );
      }
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-1">
                  <input
                    ...
                    value={team1Sets.s1}
                    onChange={(e) =>
                      setTeam1Sets({ ...team1Sets, s1: e.target.value })
                    }
                  ...
                  <input
                    ...
                    value={team1Sets.s2}
                    onChange={(e) =>
                      setTeam1Sets({ ...team1Sets, s2: e.target.value })
                    }
                    className="px-2 py-1.5 text-sm border border-gray-300 rounded text-center focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  />
                  <input
                    ...
                    value={team1Sets.s3}
                    onChange={(e) =>
                      setTeam1Sets({ ...team1Sets, s3: e.target.value })
                    }
                    className="px-2 py-1.5 text-sm border border-gray-300 rounded text-center focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  />
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-1">
                  <input
                    ...
                    value={team2Sets.s1}
                    onChange={(e) =>
                      setTeam2Sets({ ...team2Sets, s1: e.target.value })
                    }
                    className="px-2 py-1.5 text-sm border border-gray-300 rounded text-center focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  />
                  <input
                    ...
                    value={team2Sets.s2}
                    onChange={(e) =>
                      setTeam2Sets({ ...team2Sets, s2: e.target.value })
                    }
                    className="px-2 py-1.5 text-sm border border-gray-300 rounded text-center focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  />
                  <input
                    ...
                    value={team2Sets.s3}
                    onChange={(e) =>
                      setTeam2Sets({ ...team2Sets, s3: e.target.value })
                    }
                    className="px-2 py-1.5 text-sm border border-gray-300 rounded text-center focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  />
                </div>
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className="bg-[#3B3BB7] text-white">
                <tr>
                  <th className="py-2 px-2 text-left font-medium text-xs uppercase w-10">
                    Pos
                  </th>
                  <th className="py-2 px-2 text-left font-medium text-xs uppercase sticky left-0 bg-[#3B3BB7] z-10 w-32">
                    Team
                  </th>
                  <th className="py-2 px-2 text-left font-medium text-xs uppercase">
                    M
                  </th>
                  <th className="py-2 px-2 text-left font-medium text-xs uppercase">
                    W
              ...
              <tbody className="divide-y divide-gray-100">
                {teams.map((team, index) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-2 font-medium text-gray-900 sticky left-0 bg-inherit z-10 truncate max-w-[120px] shadow-[4px_0_4px_-4px_rgba(0,0,0,0.1)]">
                      {team.name}
                    </td>
                    ...
                    <td className="py-2 px-2 font-bold text-[#3B3BB7]">
                      {team.points}
                    </td>
                    <td className="py-2 px-2 text-gray-700">
                      {team.setRatio.toFixed(2)}
                    </td>
                    <td className="py-2 px-2 text-gray-700">
                      {team.pointRatio.toFixed(2)}
                    </td>
                  </tr>
                ))}
              </tbody>
